...
function _render_textures!(buf::Buffer, area::Rect, m::EffectsModel)
    th = theme()
    block = Block(;
        title="Texture Fills", border_style=tstyle(:border), title_style=tstyle(:title; bold=true)
    )
    inner_area = render(block, area, buf)
    inner_area.height < 4 && return nothing
    ...
    noise_rect = Rect(
        inner_area.x, inner_area.y + half_h, inner_area.width, inner_area.height - half_h
    )

    # Gradient fill
    dir = GRADIENT_DIRS[m.gradient_idx]
    fill_gradient!(buf, grad_rect, to_rgb(th.primary), to_rgb(th.accent); direction=dir)
    ...
    if in_bounds(buf, lx, ly)
        set_string!(buf, lx, ly, dir_label, Style(; fg=Color256(0), bg=th.primary, bold=true))
    end

    # Noise fill
    fill_noise!(
        buf, noise_rect, to_rgb(th.secondary), to_rgb(th.accent), m.tick; scale=0.2, speed=0.03
    )
    ...
    if in_bounds(buf, nx, ny)
        set_string!(buf, nx, ny, noise_label, Style(; fg=Color256(0), bg=th.secondary, bold=true))
    end
end
...
function _render_widgets!(buf::Buffer, area::Rect, m::EffectsModel)
    th = theme()
    block = Block(;
        title="Widget Effects", border_style=tstyle(:border), title_style=tstyle(:title; bold=true)
    )
    inner_area = render(block, area, buf)
    inner_area.height < 6 && return nothing
    ...
    render(Gauge(v; tick=m.tick, block=nothing, label="$(round(Int, v * 100))%"), gauge_rect, buf)
    y += 2
    ...
    if y + 1 <= bottom(inner_area)
        ...
        render(TextInput(; text="cursor breathes", tick=m.tick, focused=true), ti_rect, buf)
        y += 2
    end
    ...
    if y + 1 <= bottom(inner_area)
        ...
        render(TextInput(; text="static cursor", tick=nothing, focused=true), ti2_rect, buf)
    end
end
...
function _render_glow!(buf::Buffer, area::Rect, m::EffectsModel)
    th = theme()
    block = Block(;
        title="Glow & Flicker", border_style=tstyle(:border), title_style=tstyle(:title; bold=true)
    )
    inner_area = render(block, area, buf)
    inner_area.width < 4 && return nothing
    inner_area.height < 4 && return nothing
    ...
    for row in inner_area.y:bottom(inner_area)
        for col in inner_area.x:right(inner_area)
            ...
            g = glow(col, row, cx, cy; radius=Float64(max(w, h)) * 0.6, falloff=2.0)
            brightness = clamp(g * fl, 0.0, 1.0)

            # Map brightness to shade character
            ci = clamp(
                round(Int, brightness * (length(shade_chars) - 1)) + 1, 1, length(shade_chars)
            )
            ch = shade_chars[ci]
...
function _render_modal!(buf::Buffer, area::Rect, m::EffectsModel)
    th = theme()
    block = Block(;
        title="Modal", border_style=tstyle(:border), title_style=tstyle(:title; bold=true)
    )
    inner_area = render(block, area, buf)
    inner_area.width < 12 && return nothing
    inner_area.height < 6 && return nothing
...
function view(m::EffectsModel, f::Frame)
    ...
    return render(
        StatusBar(;
            left=[
                Span(
                    "  [g]gradient:$(dir_name) [space]$(pause_label) [tab]modal:$(sel_label) ",
                    tstyle(:text_dim),
                ),
            ],
...
function effects_demo(; theme_name=nothing)
    ...
    return app(EffectsModel(); fps=30)
end
