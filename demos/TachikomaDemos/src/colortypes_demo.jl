...
function update!(m::ColorTypesModel, evt::KeyEvent)
    return (evt.key == :escape || (evt.key == :char && evt.char == 'q')) && (m.quit = true)
end

function _run_tests!(m::ColorTypesModel)
    m.ran && return nothing
    m.ran = true
    t = m.results
    ...
    try
        ...
        push!(
            t,
            ColorTest(
                "to_rgb(RGB{N0f8}(1,0,0)) → ($(c.r),$(c.g),$(c.b))",
                c == ColorRGB(0xff, 0x00, 0x00),
                c,
            ),
        )
    catch e
    ...
    try
        ...
        push!(
            t,
            ColorTest(
                "to_rgb(RGB(0.5,0.25,0.75)) → ($(c.r),$(c.g),$(c.b))",
                c.r == 128 && c.g == 64 && c.b == 191,
                c,
            ),
        )
    catch e
    ...
    try
        ...
        push!(
            t,
            ColorTest(
                "to_rgba(RGBA(0,1,0,0.5)) → ($(c.r),$(c.g),$(c.b),a=$(c.a))",
                c.g == 0xff && c.a == 0x80,
    ...
    try
        ...
        push!(
            t, ColorTest("to_rgba(RGB(0,0.5,1)) → a=$(c.a)", c.a == 0xff, ColorRGB(c.r, c.g, c.b))
        )
    catch e
    ...
    try
        ...
        push!(
            t,
            ColorTest(
                "to_colortype(ColorRGB(255,128,0)) → $(c)",
                c == RGB{N0f8}(1.0, reinterpret(N0f8, 0x80), 0.0),
                orig,
            ),
        )
    catch e
    ...
    try
        ...
        push!(
            t,
            ColorTest(
                "to_colortype(ColorRGBA(64,192,255,128)) → $(c)",
                c.g == reinterpret(N0f8, 0xc0),
                ColorRGB(orig.r, orig.g, orig.b),
            ),
        )
    catch e
    ...
    try
        ...
        push!(t, ColorTest("Roundtrip (0xAB,0xCD,0xEF) → RGB{N0f8} → ColorRGB", back == orig, orig))
    catch e
    ...
    try
        ...
        push!(
            t,
            ColorTest(
                "Roundtrip RGBA (0xDE,0xAD,0xBE,a=0xEF)",
                back == orig,
                ColorRGB(orig.r, orig.g, orig.b),
            ),
        )
