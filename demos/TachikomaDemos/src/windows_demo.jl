...
const _WDM_PALETTES = Dict(
    :signals => (
        border=ColorRGB(0x6c, 0xd0, 0xf0),
        accent=ColorRGB(0x4e, 0xb8, 0xe0),
        dim=ColorRGB(0x20, 0x40, 0x55),
    ),
    :inputs => (
        border=ColorRGB(0xf0, 0x80, 0xd0),
        accent=ColorRGB(0xe0, 0x60, 0xb0),
        dim=ColorRGB(0x55, 0x20, 0x45),
    ),
    :data => (
        border=ColorRGB(0xa0, 0xe0, 0x70),
        accent=ColorRGB(0x80, 0xd0, 0x50),
        dim=ColorRGB(0x30, 0x50, 0x20),
    ),
    :form => (
        border=ColorRGB(0xf0, 0xc0, 0x50),
        accent=ColorRGB(0xe0, 0xa0, 0x30),
        dim=ColorRGB(0x55, 0x45, 0x15),
    ),
    :noise => (
        border=ColorRGB(0xb0, 0x90, 0xf0),
        accent=ColorRGB(0x90, 0x70, 0xe0),
        dim=ColorRGB(0x30, 0x20, 0x50),
    ),
)
...
function _ensure_windows!(m::WindowsDemoModel, area::Rect)
    ...
    m.inputs_form = Form(
        [
            ...
            FormField(
                "Action",
                Button("Run Task"; button_style=ButtonStyle(; decoration=BorderedButton())),
            ),
        ];
        submit_label="Apply",
        bordered_submit=true,
    ...
    m.datatable = DataTable(
        ...
        [Any[nodes...], Any[cpu...], Any[mem...], Any[statuses...]];
        selected=1,
    )
    ...
    push!(
        ...
        FloatingWindow(;
            ...
            width=46,
            height=12,
            border_color=_WDM_PALETTES[:signals].border,
            box=BOX_ROUNDED,
        ),
    )
    push!(
        ...
        FloatingWindow(;
            ...
            width=40,
            height=18,
            border_color=_WDM_PALETTES[:inputs].border,
            box=BOX_ROUNDED,
    ...
    push!(
        ...
        FloatingWindow(;
            ...
            width=38,
            height=14,
            border_color=_WDM_PALETTES[:data].border,
            box=BOX_HEAVY,
            content=m.datatable,
        ),
    )
    push!(
        ...
        FloatingWindow(;
            ...
            width=36,
            height=11,
            border_color=_WDM_PALETTES[:form].border,
            box=BOX_DOUBLE,
            content=m.form,
        ),
    )
    push!(
        ...
        FloatingWindow(;
            ...
            width=28,
            height=10,
            border_color=_WDM_PALETTES[:noise].border,
            box=BOX_ROUNDED,
        ),
    )
    return m.wm = wm
...
function _close_detail!(m::WindowsDemoModel, id::Symbol)
    ...
    return delete!(m.detail_win_ids, id)
end
...
function _open_detail_popup!(m::WindowsDemoModel, dt::DataTable, row::Int)
    ...
    win = FloatingWindow(;
        ...
        height=ph,
        box=BOX_DOUBLE,
        border_color=ColorRGB(0xff, 0xd0, 0x60),
        resizable=false,
        closeable=true,
    )
    ...
    let rd = row_data
        win.on_render =
            (inner, buf, focused, frame) -> begin
                (inner.width < 3 || inner.height < 2) && return nothing
                for (i, (label, val)) in enumerate(rd)
                    y = inner.y + i - 1
                    y > bottom(inner) && break
                    set_string!(
                        buf, inner.x + 1, y, label * ":", Style(; fg=ColorRGB(0xa0, 0xa0, 0xa0))
                    )
                    set_string!(
                        buf,
                        inner.x + 10,
                        y,
                        val,
                        Style(; fg=ColorRGB(0xff, 0xff, 0xff), bold=true),
                    )
                end
                hy = bottom(inner)
                set_string!(
...
function _wdm_format(col, row::Int)
    ...
    return string(col.values[row])
end

function Tachikoma.update!(m::WindowsDemoModel, evt::MouseEvent)
    return m.wm !== nothing && handle_mouse!(m.wm, evt)
end
...
function Tachikoma.view(m::WindowsDemoModel, f::Frame)
    ...
    for (i, data) in enumerate(m.spark_data)
        v = 0.5 + 0.4 * sin(m.tick * 0.05 * i + i * 1.2) + 0.1 * sin(m.tick * 0.13 + i * 3.7)
    ...
    return set_string!(
        buf, area.x + 1, footer_y, hint, Style(; fg=brighten(bg, 0.8), bg=footer_bg, bold=true)
    )
end

# ── Per-window content rendering ─────────────────────────────────────

function _render_signals!(
    m::WindowsDemoModel, colors::NamedTuple, inner::Rect, buf::Buffer, focused::Bool, tick::Int
)
    (inner.width < 3 || inner.height < 3) && return nothing
    ...
    if inner.height >= 8
        ...
        if label_y2 + 2 <= bottom(inner)
            ...
            render(
                Sparkline(data2; style=Style(; fg=brighten(colors.accent, 0.2)), max_val=1.0),
                spark_rect2,
                buf,
            )
        end
...
function _render_noise!(colors::NamedTuple, inner::Rect, buf::Buffer, focused::Bool, tick::Int)
...
function windows_demo()
    return app(WindowsDemoModel(); fps=30)
end
