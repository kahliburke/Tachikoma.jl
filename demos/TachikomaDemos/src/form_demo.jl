...
function _build_form(tick)
    return Form(
        ...
        submit_label="Submit",
        block=Block(; title="Form", border_style=tstyle(:border), title_style=tstyle(:title)),
        tick=tick,
    )
end
...
function update!(m::FormModel, evt::KeyEvent)
    ...
    is_submit_press =
        cur === m.form.submit_button && (evt.key == :enter || (evt.key == :char && evt.char == ' '))
...
function _render_preview!(buf::Buffer, area::Rect, m::FormModel)
    ...
    for (label, val) in sort(collect(vals); by=first)
        ...
        set_string!(buf, inner.x, y, "$(label): ", label_style; max_x=right(inner))
        vx = inner.x + length(label) + 2
        set_string!(buf, vx, y, display_val, value_style; max_x=right(inner))
        y += 1
    end
end
