...
function simulate_sysmon!(m::SysmonModel)
    ...
    for i in eachindex(m.cpu_cores)
        base = 0.15 + 0.1 * i
        m.cpu_cores[i] = clamp(
            base + 0.2 * sin(t * (0.5 + 0.1i)) + 0.1 * sin(t * (1.7 + 0.3i)) + 0.03 * randn(),
            0.01,
            0.99,
        )
    end
    ...
    m.mem_used = clamp(6.0 + 1.5 * sin(t * 0.2) + 0.3 * sin(t * 0.7), 2.0, 14.0)
    push!(m.mem_history, m.mem_used / m.mem_total)
    length(m.mem_history) > 200 && popfirst!(m.mem_history)

    rx = clamp(50.0 + 30 * sin(t * 0.8) + 15 * sin(t * 2.1) + 5 * randn(), 0.0, 100.0)
    tx = clamp(20.0 + 10 * sin(t * 0.6) + 8 * sin(t * 1.9) + 3 * randn(), 0.0, 60.0)
    push!(m.net_rx_history, rx)
    push!(m.net_tx_history, tx)
    length(m.net_rx_history) > 200 && popfirst!(m.net_rx_history)
    return length(m.net_tx_history) > 200 && popfirst!(m.net_tx_history)
end

function draw_line_plot!(canvas::Canvas, data::Vector{Float64}, max_val::Float64)
    dw = canvas.width * 2
    ...
    return render(
        StatusBar(;
            left=[
                ...
                Span("  MEM: $(round(m.mem_used;digits=1))/$(m.mem_total)G", tstyle(:secondary)),
            ],
...
function view_overview(m::SysmonModel, area::Rect, buf::Buffer)
    ...
    left_rows = split_layout(
        Layout(
            Vertical,
            [Fixed(1), Fixed(length(m.cpu_cores) + 2), Fixed(1), Fixed(3), Fixed(1), Fill()],
        ),
        cols[1],
    )
    length(left_rows) < 6 && return nothing

    # CPU header
    set_string!(buf, left_rows[1].x + 1, left_rows[1].y, "CPU Cores", tstyle(:text; bold=true))
    ...
    bars = [
        ...
        elseif v > 0.5
            ...
            tstyle(:primary)
        end) for (i, v) in enumerate(m.cpu_cores)
    ]
    render(BarChart(bars; max_val=100.0, label_width=7), left_rows[2], buf)

    # Memory header
    set_string!(buf, left_rows[3].x + 1, left_rows[3].y, "Memory", tstyle(:text; bold=true))
    ...
    if left_rows[6].height >= 8
        set_string!(buf, left_rows[5].x + 1, left_rows[5].y, "Calendar", tstyle(:text; bold=true))
        render(Calendar(), left_rows[6], buf)
    end

    # Right: CPU history braille plot + memory history
    right_rows = split_layout(Layout(Vertical, [Fixed(1), Fill(), Fixed(1), Fill()]), cols[2])
    length(right_rows) < 4 && return nothing

    set_string!(
        buf, right_rows[1].x + 1, right_rows[1].y, "CPU History (avg)", tstyle(:text; bold=true)
    )

    # CPU history canvas
    plot1 = right_rows[2]
    if plot1.width >= 4 && plot1.height >= 2
        canvas1 = Canvas(plot1.width, plot1.height; style=tstyle(:primary))
        draw_line_plot!(canvas1, m.cpu_history, 1.0)
        render(canvas1, plot1, buf)
    end

    set_string!(
        buf, right_rows[3].x + 1, right_rows[3].y, "Memory History", tstyle(:text; bold=true)
    )

    plot2 = right_rows[4]
    if plot2.width >= 4 && plot2.height >= 2
        canvas2 = Canvas(plot2.width, plot2.height; style=tstyle(:secondary))
...
function view_processes(m::SysmonModel, area::Rect, buf::Buffer)
    ...
    render(
        Table(
            ...
            SYSMON_PROCS;
            block=Block(;
                title="processes ($(length(SYSMON_PROCS)))",
