...
function _unicode_view_paragraph(inner::Rect, buf::Buffer)
    ...
    blk_l = Block(;
        title="Alignment (no wrap)", border_style=tstyle(:border), title_style=tstyle(:title)
    )
    ...
    blk_r = Block(;
        title="Grapheme catalogue", border_style=tstyle(:border), title_style=tstyle(:title)
    )
    inner_r = render(blk_r, halves[2], buf)
    p2 = Paragraph(join(_UNICODE_TEST_LINES, "\n"))
    return render(p2, inner_r, buf)
end

function _unicode_view_wrap(inner::Rect, buf::Buffer)
    blk = Block(;
        title="Word wrap with combining marks + wide chars",
    ...
    text =
        "The café serves naïve customers. Letters like ṅ and n̈ " *
...
function _unicode_view_table(inner::Rect, buf::Buffer)
    ...
    tbl = Table(
        ...
        rows;
        widths=[50],
        block=Block(; title="Table", border_style=tstyle(:border), title_style=tstyle(:title)),
    )
    return render(tbl, inner, buf)
end
...
function _unicode_view_aligned(inner::Rect, buf::Buffer)
    ...
    for (i, al) in enumerate([align_left, align_center, align_right])
        ...
        p = Paragraph(
            text;
            alignment=al,
            block=Block(; title=label, border_style=tstyle(:border), title_style=tstyle(:title)),
        )
...
function view(m::UnicodeDemoModel, f::Frame)
    ...
    # Tabs
    tabs = TabBar(["Paragraph", "Word Wrap", "Table", "Alignment"]; active=m.tab, focused=true)
    ...
    render(
        StatusBar(;
            ...
            right=[
                Span("ṅ n̈ 你好 café  ", tstyle(:text_dim)), Span("[q] quit ", tstyle(:text_dim))
            ],
        ),
        footer_area,
        buf,
...
function unicode_demo()
    return app(UnicodeDemoModel(); fps=30)
end
