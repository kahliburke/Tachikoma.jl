...
function view(m::DemoModel, f::Frame)
    ...
    x2 = set_string!(buf, x + 2, y, "$(th.name) theme", tstyle(:text_dim))
    set_string!(
        buf, x2 + 1, y, "$(DOT) $(f.area.width)×$(f.area.height)", tstyle(:text_dim; dim=true)
    )
    ...
    y += 1
    bars = [0.8, 0.6, 0.9, 0.3, 0.7, 0.5, 0.85, 0.4, 0.65, 0.75, 0.2, 0.95]
    for (i, v) in enumerate(bars)
        ...
        c = if animations_enabled()
            color_wave(m.tick, i, (th.primary, th.accent, th.secondary); speed=0.03, spread=0.4)
        else
            th.primary
        end
    ...
    # ── gradient blocks ──
    y += 2
    set_string!(buf, x, y, "density", tstyle(:text; bold=true))
    y += 1
    for (i, ch) in enumerate(BLOCKS)
        for j in 1:5
            set_char!(buf, x + (i - 1) * 5 + j - 1, y, ch, tstyle(:primary))
        end
    end

    # ── box style showcase ──
    y += 2
    set_string!(buf, x, y, "borders", tstyle(:text; bold=true))
    y += 1
    boxes = [
        ("rounded", BOX_ROUNDED), ("heavy", BOX_HEAVY), ("double", BOX_DOUBLE), ("plain", BOX_PLAIN)
    ]
    bx = x
    for (label, box) in boxes
        ...
        if bx + w <= right(content)
            mini = Block(;
                title="$label", box=box, border_style=tstyle(:border), title_style=tstyle(:text_dim)
            )
    ...
    if y <= bottom(content)
        for cx in x:(right(content) - 1)
            set_char!(buf, cx, y, SCANLINE, tstyle(:border; dim=true))
        end
    end
    ...
    iy = bottom(content)
    if iy > y
        set_string!(buf, x, iy, "[↑↓/jk]theme [q/Esc]quit", tstyle(:text_dim; dim=true))
    end

    # ── theme selector sidebar ──
    sb_block = Block(; title="Themes", border_style=tstyle(:border), title_style=tstyle(:title))
    ...
    for (i, t) in enumerate(ALL_THEMES)
        ...
        if i == m.theme_idx
            ...
            set_string!(buf, sb_inner.x + 2, sy, t.name, tstyle(:text_dim); max_x=right(sb_inner))
        end
    end
end
