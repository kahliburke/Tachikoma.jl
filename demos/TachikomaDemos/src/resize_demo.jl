...
function update!(m::ResizeDemoModel, evt::KeyEvent)
    ...
    return evt.key == :down && (m.list_selected = min(m.list_selected + 1, n))
end
...
function update!(m::ResizeDemoModel, evt::MouseEvent)
    ...
    # Scroll
    return m.list_offset = list_scroll(evt, m.list_offset, length(m.list_items), m.list_visible_h)
end

function copy_rect(m::ResizeDemoModel)
    isempty(m.pane_rects) && return nothing
    return m.pane_rects[clamp(m.focused_pane, 1, length(m.pane_rects))]
end
...
function view(m::ResizeDemoModel, f::Frame)
    ...
    p1_block = Block(;
        title="Pane 1: List (click to select)", border_style=p1_border, title_style=tstyle(:title)
    )
    ...
    for i in 1:m.list_visible_h
        ...
        if idx == m.list_selected
            ...
            set_string!(buf, p1_inner.x + 2, y, m.list_items[idx], tstyle(:accent; bold=true))
        else
            set_string!(buf, p1_inner.x + 2, y, m.list_items[idx], tstyle(:text))
        end
    end

    # ── Pane 2: info ──
    p2_border = m.focused_pane == 2 ? tstyle(:border_focus) : tstyle(:border)
    p2_block = Block(; title="Pane 2: Info", border_style=p2_border, title_style=tstyle(:title))
    ...
    p3_block = Block(; title="Instructions", border_style=p3_border, title_style=tstyle(:title))
    ...
    return render_resize_handles!(buf, m.hlayout)
end
