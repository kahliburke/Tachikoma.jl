...
const DT_ROLES = ["Major", "Operator", "Analyst", "Sniper", "Engineer", "Medic", "Hacker", "Scout"]
const DT_REGIONS = [
    "Tokyo-3",
    "Neo Hong Kong",
    "Newport",
...
function _build_datatable()
    ...
    return DataTable(
        [
            ...
            DataColumn(
                "Latency", collect(Any, latency); align=col_right, format=v -> string(v, " ms")
            ),
        ];
        selected=1,
        show_scrollbar=true,
...
function _build_wide_datatable()
    ...
    return DataTable(
        [
            ...
            DataColumn(
                "Uptime", collect(Any, uptime); align=col_right, format=v -> string(v, " hrs")
            ),
            DataColumn(
                "Memory", collect(Any, memory); align=col_right, format=v -> string(v, " MB")
            ),
...
function _active_dt(m::DataTableModel)
    return m.mode == 1 ? m.dt : m.dt_wide
end
...
function update!(m::DataTableModel, evt::MouseEvent)
    return handle_mouse!(_active_dt(m), evt)
end
...
function view(m::DataTableModel, f::Frame)
    ...
    dt.block = Block(;
        title="Roster$(sort_info)", border_style=tstyle(:border), title_style=tstyle(:title)
    )
    ...
    footer = if m.mode == 1
        StatusBar(;
            left=[
                Span("  [↑↓]navigate [PgUp/PgDn]page [1-4]sort [m]wide mode ", tstyle(:text_dim)),
            ],
        ...
        StatusBar(;
            left=[
                Span(
                    "  [↑↓]navigate [←→]scroll [1-8]sort [d]detail [m]compact mode ",
                    tstyle(:text_dim),
                ),
            ],
            right=[Span("[q/Esc]quit ", tstyle(:text_dim))],
    ...
    return render(footer, footer_area, buf)
end

function datatable_demo(; theme_name=nothing)
    theme_name !== nothing && set_theme!(theme_name)
    return app(DataTableModel(); fps=30)
end
