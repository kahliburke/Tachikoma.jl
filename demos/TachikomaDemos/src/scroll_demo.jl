...
function Tachikoma.render(sc::ScrollCanvas, rect::Rect, buf::Buffer)
    ...
    render(
        Block(;
            title="Widget Scroll Demo — $(rect.width)×$(rect.height) virtual cells",
            border_style=tstyle(:accent),
            title_style=tstyle(:accent; bold=true),
        ),
        Rect(rect.x, rect.y, bw, 3),
        buf,
    )
    set_string!(
        buf,
    ...
    bars1 = [
        BarEntry(d, 10 + round(Int, 15 * sin(t * 0.04 + i * 0.9))) for
        (i, d) in enumerate(_SCROLL_DAYS)
    ]
    render(
        BarChart(
            bars1;
    ...
    bars2 = [
        BarEntry("US-E", 35 + round(Int, 10 * sin(t * 0.03))),
    ...
    render(
        Table(
            ...
            _SCROLL_SERVICES;
            block=Block(;
                title="Service Health Dashboard ($(length(_SCROLL_SERVICES)) services)",
                border_style=tstyle(:border),
            ),
            header_style=tstyle(:title; bold=true),
        ),
        Rect(rect.x, y, 90, length(_SCROLL_SERVICES) + 4),
        buf,
    )

    # Calendars to the right
    for (ci, (yr, mo, title)) in
        enumerate([(2026, 3, "March 2026"), (2026, 4, "April 2026"), (2026, 5, "May 2026")])
        cx = rect.x + 95 + (ci - 1) * 26
        render(
            Calendar(yr, mo; block=Block(; title=title, border_style=tstyle(:border))),
            Rect(cx, y, 24, 10),
            buf,
        )
    end
    ...
    for (bi, (title, offset)) in enumerate([
        ...
        bx = rect.x + (bi - 1) * 48
        entries = [BarEntry("B$j", 5 + round(Int, 20 * sin(t * 0.03 + j + offset))) for j in 1:7]
        render(
            BarChart(
                entries;
    ...
    y += 14
    labels3 = [
        "IOPS Read", "IOPS Write", "Throughput MB/s", "Queue Depth", "Latency µs", "Utilization"
    ]
    ...
    render(
        Table(
            ...
            endpoints;
            block=Block(;
                title="Endpoint Performance ($(length(endpoints)) endpoints)",
                border_style=tstyle(:border),
            ),
            header_style=tstyle(:title; bold=true),
        ),
        Rect(rect.x, y, 85, length(endpoints) + 4),
        buf,
    )
    ...
    labels4 = [
        "GC Pause ms", "Alloc Rate", "Thread Count", "Goroutines", "Open Files", "TCP Connections"
    ]
    ...
    return set_string!(
        buf, rect.x + 2, y + 1, "Press [Home] to return to the top.", tstyle(:text_dim)
    )
end
...
@kwdef mutable struct ScrollDemoModel <: Model
    quit::Bool = false
    tick::Int = 0
    scroll::WidgetScroll = WidgetScroll(
        ScrollCanvas();
        virtual_width=200,
        virtual_height=120,
        block=Block(
            title="Viewport — drag to pan, scroll wheel, arrow keys, Home to reset",
            border_style=tstyle(:border),
...
function update!(m::ScrollDemoModel, evt::KeyEvent)
    (evt.key == :escape || (evt.key == :char && evt.char == 'q')) && (m.quit=true; return nothing)
    return handle_key!(m.scroll, evt)
end

function update!(m::ScrollDemoModel, evt::MouseEvent)
    return handle_mouse!(m.scroll, evt)
end
...
function view(m::ScrollDemoModel, f::Frame)
    ...
    return render(
        StatusBar(;
            left=[
                Span(
                    "  offset: ($ox,$oy) virtual: 200×120$drag_indicator  [←→↑↓]scroll [drag]pan [Home]reset ",
                    tstyle(:text_dim),
                ),
            ],
