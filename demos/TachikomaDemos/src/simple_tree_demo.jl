    ...
    root = TreeNode(
        ...
        [
            TreeNode(
                ...
                [
                    TreeNode("thing"),
                    TreeNode("A"; content=0x32ab),
                    TreeNode("C"; content="content of C"),
                ],
            ),
            TreeNode(
                "Component",
                [
                    TreeNode("thing"; content="A string of content!"),
                    TreeNode("B"),
                    TreeNode("D"; content="content of D"),
                ],
            ),
            TreeNode(
                "Another component",
                [
                    TreeNode("thing"; content=-3.4e4),
                    TreeNode("E"; content=Dict("a"=>1, "b"=>2, "c"=>3)),
                ],
            ),
        ],
    )
...
function update!(m::TreeState, event::KeyEvent)
    ...
    return handle_key!(m.tree, event)
end

function update!(m::TreeState, event::MouseEvent)
    return handle_mouse!(m.tree, event)
end
...
function view(m::TreeState, f::Frame)
    ...
    spn = Paragraph(
        "value: $(value(m.tree))\n\nselected node: $(node_selection)\n\nnode content: $(content_selection)";
        wrap=char_wrap,
    )

    render(m.tree, innerL, f.buffer)
    return render(spn, innerR, f.buffer)
end
