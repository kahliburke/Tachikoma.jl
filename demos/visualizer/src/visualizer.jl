...
function load_audio(filepath::AbstractString)
    ...
    Sys.which("ffmpeg") !== nothing || error(
        "ffmpeg not found. Install it to load $ext files, " *
        "or convert manually: ffmpeg -i input$ext output.wav",
    )
    tmpfile = tempname() * ".wav"
    try
        run(pipeline(
            `ffmpeg -y -i $filepath -f wav -acodec pcm_s16le
             -ar 44100 $tmpfile`,
            devnull,
            devnull,
        ))
...
function c256_rgb(code::UInt8)
    ...
    elseif c < 232
        i = c - 16
        return RGB{N0f8}((i ÷ 36) / 5, ((i ÷ 6) % 6) / 5, (i % 6) / 5)
    else
        v = ((c - 232) * 10 + 8) / 255
...
function start_playback!(m::VisualizerModel)
    m.playing = true
    m.paused = false
    return m.play_task = @async begin
        try
            nch = min(m.nchannels, 2)
            stream = PortAudioStream(0, nch; samplerate=Float64(m.samplerate))
            m.stream = stream
            ...
            while m.playing && m.position[] <= total
                ...
                analyze_chunk!(m, m.samples[win_start:play_end, 1])

                # Play only the hop-sized chunk
                chunk = m.samples[pos:play_end, 1:nch]
                write(stream, chunk)
                Threads.atomic_add!(m.position, play_end - pos + 1)
            end
            close(stream)
            m.stream = nothing
            m.playing = false
...
function analyze_chunk!(m::VisualizerModel, chunk::Vector{Float64})
    ...
    return length(m.spec_history) > m.max_history && popfirst!(m.spec_history)
end
...
function cycle_up(val, options)
    ...
    return idx >= length(options) ? options[end] : options[idx + 1]
end

function cycle_down(val, options)
    idx = findfirst(==(val), options)
    idx === nothing && return first(options)
    return idx <= 1 ? options[1] : options[idx - 1]
end
...
function update!(m::VisualizerModel, evt::KeyEvent)
    if evt.key == :char
        ...
        # Gain
        evt.char == '+' && (m.sensitivity = min(5.0, m.sensitivity * 1.3))
        evt.char == '=' && (m.sensitivity = min(5.0, m.sensitivity * 1.3))
        evt.char == '-' && (m.sensitivity = max(0.2, m.sensitivity / 1.3))
        # Display mode
        evt.char == '1' && (m.mode = 1)
        evt.char == '2' && (m.mode = 2)
        evt.char == '3' && (m.mode = 3)
        ...
        if evt.char == 'o'
            m.oversample = cycle_down(m.oversample, OVERSAMPLE_STEPS)
        elseif evt.char == 'O'
            m.oversample = cycle_up(m.oversample, OVERSAMPLE_STEPS)
        end
...
function draw_bars_px!(
    ...
    for i in 1:n
        ...
        peak_h = round(Int, clamp(peaks[i] / 1.5, 0.0, 1.0) * h * 0.88)
        ...
        for rfb in 1:bar_h
            ...
            color = if frac > 0.82
                lerp_rgb(pal.warning, pal.error_c, (frac - 0.82) / 0.18)
            elseif frac > 0.50
                lerp_rgb(pal.primary, pal.warning, (frac - 0.50) / 0.32)
            else
                lerp_rgb(scale_rgb(pal.primary, 0.35), pal.primary, frac / 0.50)
            end
...
function draw_scope_px!(img::AbstractMatrix{RGB{N0f8}}, waveform::Vector{Float64}, pal::VisPalette)
    h, w = size(img)
    n = length(waveform)
    (n == 0 || h < 2 || w < 2) && return nothing
...
function spec_heatmap(val::Float64, pal::VisPalette)
    ...
    val > 0.80 && return lerp_rgb(pal.warning, pal.error_c, (val - 0.80) / 0.20)
    val > 0.50 && return lerp_rgb(pal.primary, pal.warning, (val - 0.50) / 0.30)
    val > 0.20 && return lerp_rgb(pal.secondary, pal.primary, (val - 0.20) / 0.30)
    return lerp_rgb(pal.bg, pal.secondary, (val - 0.04) / 0.16)
end

function draw_spectrogram_px!(
    img::AbstractMatrix{RGB{N0f8}}, history::Vector{Vector{Float64}}, pal::VisPalette
)
    isempty(history) && return nothing
    h, w = size(img)
    (h < 2 || w < 1) && return nothing
    ...
    for px_col in 1:w
        ...
        for py in 0:(usable_h - 1)
            ...
            val = clamp(
                (v00 * (1 - ft) + v10 * ft) * (1 - bt) + (v01 * (1 - ft) + v11 * ft) * bt, 0.0, 1.0
            )
...
function view(m::VisualizerModel, f::Frame)
    ...
    block = Block(;
        title="$(m.filename)", border_style=tstyle(:border), title_style=tstyle(:title; bold=true)
    )
    content = render(block, f.area, buf)
    x0 = content.x
    status_y = bottom(content)

    vis_area = Rect(x0, content.y, content.width, max(1, content.height - 1))
    ...
    if pw > 0 && ph > 0
        ...
        elseif m.mode == 2
            h_split = (ph * 2) ÷ 3
            draw_bars_px!(@view(m.pixel_buf[1:h_split, :]), m.spectrum, m.peaks, pal)
            draw_scope_px!(@view(m.pixel_buf[(h_split + 1):end, :]), m.waveform, pal)
        elseif m.mode == 3
            h_split = (ph * 2) ÷ 3
            draw_bars_px!(@view(m.pixel_buf[1:h_split, :]), m.spectrum, m.peaks, pal)
            draw_spectrogram_px!(@view(m.pixel_buf[(h_split + 1):end, :]), m.spec_history, pal)
        end
    ...
    if status_y <= bottom(f.area)
        ...
        time_str = string(pm, ":", lpad(ps, 2, '0'), " / ", dm, ":", lpad(ds, 2, '0'))
        state = if m.paused
            "paused"
        elseif m.playing
            "playing"
        ...
        cx = set_string!(buf, x0, status_y, state, tstyle(m.paused ? :warning : :success))
        cx = set_string!(buf, cx + 1, status_y, time_str, tstyle(:text_dim))
        ...
        cx = set_string!(buf, cx + 1, status_y, fft_str, tstyle(:text_dim))

        inst = " [1-3]mode [+-]gain [[]]]bands" * " [,.]fft [oO]oversample [space]pause"
...
"""
function visualizer(
    filepath::AbstractString; theme_name=:kokaku, num_bands=48, chunk_size=2048, oversample=4
)
