...
function _svg_braille_rects(
    io::IO, ch::Char, px::Int, py::Int, cell_w::Int, cell_h::Int, fg_hex::String
)
    mask = UInt32(ch) - UInt32('⠀')
    mask == 0 && return nothing
    ...
    for row in 0:3, col in 0:1
        ...
        write(
            io,
            """  <rect x="$(px + dx)" y="$(py + dy)" width="$dot_d" height="$dot_d" fill="$fg_hex"/>\n""",
        )
    end
end
...
function _svg_block_rect(
    ...
    elseif c == 0x2581
        (0.0, 0.875, 1.0, 0.125)  # ▁ lower 1/8
    elseif c == 0x2582
        (0.0, 0.75, 1.0, 0.25)    # ▂ lower 1/4
    elseif c == 0x2583
        (0.0, 0.625, 1.0, 0.375)  # ▃ lower 3/8
    elseif c == 0x2584
        (0.0, 0.5, 1.0, 0.5)      # ▄ lower half
    elseif c == 0x2585
        (0.0, 0.375, 1.0, 0.625)  # ▅ lower 5/8
    elseif c == 0x2586
        (0.0, 0.25, 1.0, 0.75)    # ▆ lower 3/4
    elseif c == 0x2587
        (0.0, 0.125, 1.0, 0.875)  # ▇ lower 7/8
    elseif c == 0x2588
        (0.0, 0.0, 1.0, 1.0)      # █ full block
    elseif c == 0x2589
        (0.0, 0.0, 0.875, 1.0)    # ▉ left 7/8
    elseif c == 0x258a
        (0.0, 0.0, 0.75, 1.0)     # ▊ left 3/4
    elseif c == 0x258b
        (0.0, 0.0, 0.625, 1.0)    # ▋ left 5/8
    elseif c == 0x258c
        (0.0, 0.0, 0.5, 1.0)      # ▌ left half
    elseif c == 0x258d
        (0.0, 0.0, 0.375, 1.0)    # ▍ left 3/8
    elseif c == 0x258e
        (0.0, 0.0, 0.25, 1.0)     # ▎ left 1/4
    elseif c == 0x258f
        (0.0, 0.0, 0.125, 1.0)    # ▏ left 1/8
    elseif c == 0x2590
        (0.5, 0.0, 0.5, 1.0)      # ▐ right half
    elseif c == 0x2591  # ░ light shade — stippled pattern
        write(
            io,
            """  <rect x="$px" y="$py" width="$cell_w" height="$cell_h" fill="$fg_hex" mask="url(#m1)"/>\n""",
        )
        return true
    elseif c == 0x2592  # ▒ medium shade — checkerboard pattern
        write(
            io,
            """  <rect x="$px" y="$py" width="$cell_w" height="$cell_h" fill="$fg_hex" mask="url(#m2)"/>\n""",
        )
        return true
    elseif c == 0x2593  # ▓ dark shade — dense stipple pattern
        write(
            io,
            """  <rect x="$px" y="$py" width="$cell_w" height="$cell_h" fill="$fg_hex" mask="url(#m3)"/>\n""",
        )
        return true
    elseif c == 0x2594
        (0.0, 0.0, 1.0, 0.125)    # ▔ upper 1/8
    elseif c == 0x2595
        (0.875, 0.0, 0.125, 1.0)  # ▕ right 1/8
    elseif c == 0x2596
        (0.0, 0.5, 0.5, 0.5)      # ▖ quadrant lower left
    elseif c == 0x2597
        (0.5, 0.5, 0.5, 0.5)      # ▗ quadrant lower right
    elseif c == 0x2598
        (0.0, 0.0, 0.5, 0.5)      # ▘ quadrant upper left
    elseif c == 0x259d
        (0.5, 0.0, 0.5, 0.5)      # ▝ quadrant upper right
    else
        nothing
    end
...
function export_svg(
    ...
    write(
        io, """<mask id="m1"><rect width="$img_w" height="$img_h" fill="url(#stip1)"/></mask>\n"""
    )
    write(
        io, """<mask id="m2"><rect width="$img_w" height="$img_h" fill="url(#stip2)"/></mask>\n"""
    )
    write(
        io, """<mask id="m3"><rect width="$img_w" height="$img_h" fill="url(#stip3)"/></mask>\n"""
    )
    write(io, "</defs>\n")
    ...
    if !isempty(font_path) && isfile(font_path)
        ...
        write(
            io,
            "@font-face{font-family:'EmbeddedFont';src:url('data:$mime;base64,$b64') format('$fmt');font-weight:normal;font-style:normal;}\n",
        )
        # Embed style variants if available
        for (variant, css_weight, css_style) in [
            ("Bold", "bold", "normal"),
            ...
            if !isempty(vpath) && isfile(vpath)
                ...
                write(
                    io,
                    "@font-face{font-family:'EmbeddedFont';src:url('data:$mime;base64,$vb64') format('$fmt');font-weight:$css_weight;font-style:$css_style;}\n",
                )
            end
        end
        write(
            io,
            "text{font-family:'EmbeddedFont',$font_family;font-size:$(max(1, cell_h - 2))px;white-space:pre;}\n",
        )
    else
        write(
            io,
            "text{font-family:$font_family;font-size:$(max(1, cell_h - 2))px;white-space:pre;}\n",
        )
    end
    write(io, "</style>\n")
    ...
    for (fi, cells) in enumerate(cell_snapshots)
        ...
        elseif fi == nframes
            ...
            write(
                io,
                """values="hidden;visible;hidden" keyTimes="0;$(round(frac_on, digits=6));$(round(frac_off, digits=6))"/>\n""",
            )
        end
        ...
        for cy in 1:height, cx in 1:width
            ...
            write(
                io,
                """  <rect x="$px" y="$py" width="$cell_w" height="$cell_h" fill="$bg_hex"/>\n""",
            )
        end
