...
function tokenize_python(chars::Vector{Char})::Vector{Token}
    ...
    while i <= n
        ...
        if c == '@' && (i == 1 || chars[i - 1] == ' ' || chars[i - 1] == '\t')
            ...
            i += 1
            while i <= n &&
                  (isletter(chars[i]) || isdigit(chars[i]) || chars[i] == '_' || chars[i] == '.')
                i += 1
            end
        ...
        if isdigit(c)
            ...
            if c == '0' && i + 1 <= n && chars[i + 1] in ('x', 'X', 'o', 'O', 'b', 'B')
                ...
                while i <= n && (
                    isdigit(chars[i]) ||
                    chars[i] in ('a', 'b', 'c', 'd', 'e', 'f', 'A', 'B', 'C', 'D', 'E', 'F', '_')
                )
                    i += 1
                end
            else
                while i <= n && (isdigit(chars[i]) || chars[i] == '_')
                    i += 1
                end
                if i <= n && chars[i] == '.'
                    i += 1
...
function tokenize_shell(chars::Vector{Char})::Vector{Token}
    ...
    while i <= n
        ...
        if c == '-' && i + 1 <= n && (isletter(chars[i + 1]) || chars[i + 1] == '-')
            ...
            i <= n && chars[i] == '-' && (i += 1)
            while i <= n &&
                  (isletter(chars[i]) || isdigit(chars[i]) || chars[i] == '-' || chars[i] == '_')
                i += 1
            end
            push!(tokens, Token(start, i - 1, tok_macro))
            continue
