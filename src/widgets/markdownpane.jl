...
"""
function set_markdown!(mp::MarkdownPane, source::AbstractString; width::Int=mp.last_width)
    mp.source = string(source)
    mp.last_width = width
    lines = _md_parse(
        mp.source,
        width;
        h1_style=mp.h1_style,
        h2_style=mp.h2_style,
        h3_style=mp.h3_style,
        bold_style=mp.bold_style,
        emph_style=mp.emph_style,
        code_style=mp.code_style,
        link_style=mp.link_style,
        quote_style=mp.quote_style,
        text_style=mp.text_style,
...
function handle_key!(mp::MarkdownPane, evt)
    return handle_key!(mp.pane, evt)
end

function handle_mouse!(mp::MarkdownPane, evt::MouseEvent)
    return handle_mouse!(mp.pane, evt)
end
...
function render(mp::MarkdownPane, rect::Rect, buf::Buffer)
    ...
    return render(mp.pane, rect, buf)
end
