...
function _generate_phylo_tree(preset::PhyloTreePreset)
    ...
    return PhyloTree(branches, preset.max_depth)
end

function _split_branch!(
    branches::Vector{PhyloBranch},
    preset::PhyloTreePreset,
    parent_angle::Float64,
    parent_end_r::Float64,
    depth::Int,
    seed::Float64,
)
    depth > preset.max_depth && return nothing
    ...
    for side in (-1, 1)
        ...
        push!(
            branches, PhyloBranch(parent_angle, child_angle, parent_end_r, child_end_r, depth, sway)
        )
        _split_branch!(
            branches,
            preset,
...
function _phylo_line!(
    dots::Matrix{UInt8},
    cell_depth::Matrix{Float64},
    x0::Int,
    y0::Int,
    x1::Int,
    y1::Int,
...
function _render_phylo_tree!(
    ...
    for branch in tree.branches
        ...
        if animations_enabled()
            sway =
                (noise(branch.sway_seed + tick * 0.02) - 0.5) *
                preset.sway_amount *
                (1.0 + depth_frac * 2.0)
        end
        ...
        elseif d == 1
            _phylo_line!(dots, cell_depth, x0, y0, x1, y1, depth_val, dot_w, dot_h, cw, ch, ub)
            ox = round(Int, -sin(ea))
            oy = round(Int, cos(ea))
            _phylo_line!(
                dots,
...
function _generate_clado_tree(preset::CladoPreset)
    ...
    for i in 1:n
        ...
        _build_clado!(branches, preset, a_min, a_max, 1, trait0, seed + i * 37.91)

        cum_angle += clade_span + gap_each
    end

    return CladoTree(branches, preset.max_depth, n)
end

function _build_clado!(
    branches::Vector{CladoBranch},
    preset::CladoPreset,
    a_min::Float64,
    a_max::Float64,
    depth::Int,
    parent_trait::Float64,
    seed::Float64,
)
    depth > preset.max_depth && return nothing
    ...
    if span < 0.015 || depth >= preset.max_depth
        ...
        push!(branches, CladoBranch(mid, mid, r_here, tip_r, depth, trait, sway))
        return nothing
    end
    ...
    if depth > 1 && noise(seed + 99.0) < preset.prune_prob
        mid = (a_min + a_max) / 2
        push!(branches, CladoBranch(mid, mid, r_here, r_child, depth, trait, sway))
        return nothing
    end
    ...
    push!(branches, CladoBranch(left_mid, right_mid, r_here, r_here, depth, trait, sway))
    # Radial: left child
    push!(branches, CladoBranch(left_mid, left_mid, r_here, r_child, depth, trait, sway + 1.0))
    # Radial: right child
    push!(branches, CladoBranch(right_mid, right_mid, r_here, r_child, depth, trait, sway + 2.0))
    ...
    _build_clado!(branches, preset, a_min, split_a, depth + 1, trait_l, seed + 23.71)
    return _build_clado!(branches, preset, split_a, a_max, depth + 1, trait_r, seed + 31.41)
end

# ── Drawing primitives ─────────────────────────────────────────────────

@inline function _clado_dot!(
    dots::Matrix{UInt8},
    cell_depth::Matrix{Float64},
    ...
    (0 <= x < dot_w && 0 <= y < dot_h) || return nothing
    cx = x ÷ 2 + 1
    cy = _dot_cy(y, ub)
    (1 <= cx <= cw && 1 <= cy <= ch) || return nothing
    dots[cx, cy] |= _dot_bit(x % 2, _dot_sub_y(y, ub), ub)
    cell_depth[cx, cy] = dv
    cell_trait[cx, cy] = tv
end

function _clado_bresenham!(
    dots::Matrix{UInt8},
    cell_depth::Matrix{Float64},
    cell_trait::Matrix{Float64},
    x0::Int,
    ...
    err = dx - dy
    while true
        _clado_dot!(dots, cell_depth, cell_trait, x0, y0, dv, tv, dot_w, dot_h, cw, ch, ub)
        (x0 == x1 && y0 == y1) && break
        e2 = 2 * err
        if e2 > -dy
            err -= dy
...
function _clado_arc!(
    ...
    for i in 0:n
        ...
        _clado_dot!(dots, cell_depth, cell_trait, x, y, dv, tv, dot_w, dot_h, cw, ch, ub)
    end
end
...
function _render_clado_tree!(
    ...
    for br in tree.branches
        ...
        sway = 0.0
        if animations_enabled()
            sway = (noise(br.sway_seed + tick * 0.02) - 0.5) * preset.sway_amount * (1.0 + df * 2.0)
        end
        ...
        if is_arc
            ...
            for off in offsets
                ...
                _clado_bresenham!(
                    dots, cell_depth, cell_trait, x0, y0, x1, y1, dv, tv, dot_w, dot_h, cw, ch, ub
                )
            end
        end
    end
