...
ease_in_out_cubic(t::Float64) = t < 0.5 ? 4.0 * t * t * t : (t -= 1.0; 4.0 * t * t * t + 1.0) # Horner-ish
...
function ease_out_elastic(t::Float64)
    ...
    p = 0.3
    s = p / 4.0
    return 2.0^(-10.0 * t) * sin((t - s) * 2π / p) + 1.0
end
...
function tween(from, to; duration::Int=30, easing::Function=ease_out_cubic, loop::Symbol=:none)
    return Tween(Float64(from), Float64(to), duration, 0, easing, loop)
end
...
"""
function Spring(
    target::Real; value::Real=target, stiffness::Real=180.0, damping::Union{Real,Symbol}=:critical
)
    k = Float64(stiffness)
    ...
    return Spring(Float64(target), Float64(value), 0.0, k, c)
end
...
function settled(s::Spring; threshold::Float64=0.01)
    return abs(s.velocity) < threshold && abs(s.value - s.target) < threshold
end
...
Timeline(entries::Vector{TimelineEntry}; loop::Bool=false) = Timeline(entries, 0, loop)
...
function _timeline_total_frames(tl::Timeline)
    ...
    return maximum(e.start_frame + e.tween.duration for e in tl.entries)
end
...
function val(a::Animator, name::Symbol)
    ...
    return error("Animator has no animation named :$name")
end
...
function jitter(tick::Int, seed::Int; amount::Float64=0.5, speed::Float64=0.1)
    ...
    return (noise(tick * speed + seed * 17.31) - 0.5) * 2.0 * amount
end
...
function flicker(tick::Int, seed::Int=0; intensity::Float64=0.1, speed::Float64=0.15)
    ...
    return 1.0 - intensity * n * n   # squared for bias toward bright
end
...
function drift(tick::Int, seed::Int=0; speed::Float64=0.02)
    animations_enabled() || return 0.5
    return noise(tick * speed + seed * 43.71)
end
...
"""
function glow(x::Int, y::Int, cx::Float64, cy::Float64; radius::Float64=5.0, falloff::Float64=2.0)
    d = sqrt((x - cx)^2 + (y - cy)^2)
    return clamp(1.0 - (d / radius)^falloff, 0.0, 1.0)
end
...
function brighten(c::Color256, amount::Float64)
    return brighten(to_rgb(c), amount)
end
...
function dim_color(c::ColorRGB, amount::Float64)
    amount = clamp(amount, 0.0, 1.0)
    inv = 1.0 - amount
    return ColorRGB(round(UInt8, c.r * inv), round(UInt8, c.g * inv), round(UInt8, c.b * inv))
end

function dim_color(c::Color256, amount::Float64)
    return dim_color(to_rgb(c), amount)
end
...
function hue_shift(c::ColorRGB, degrees::Float64)
    ...
    return ColorRGB(round(UInt8, r2 * 255), round(UInt8, g2 * 255), round(UInt8, b2 * 255))
end
...
function hue_shift(c::Color256, degrees::Float64)
    return hue_shift(to_rgb(c), degrees)
end
...
function fill_gradient!(
    buf::Buffer, rect::Rect, c1::AbstractColor, c2::AbstractColor; direction::Symbol=:horizontal
)
    a = to_rgb(c1)
    b = to_rgb(c2)
...
"""
function border_shimmer!(
    buf::Buffer,
    rect::Rect,
    base_color::AbstractColor,
    tick::Int;
