...
function _find_font_variant_uncached(font_path::String, variant::String)
    ...
    for (from, to) in
        [("-Regular", "-$variant"), ("-regular", "-$(lowercase(variant))"), ("Regular", variant)]
...
const _DISCOVERED_FONTS = Ref{
    Union{Nothing,Vector{NamedTuple{(:name, :path),Tuple{String,String}}}}
}(
    nothing
)
...
function _name_from_filename(fname::String)
    ...
    base = replace(
        base,
        r"[-_](Regular|Bold|Italic|Light|Medium|Thin|Black|ExtraBold|SemiBold|ExtraLight|BoldItalic|LightItalic|MediumItalic)$"i => "",
    )
...
function _is_mono_font(fname_lower::String)
    return any(kw -> occursin(kw, fname_lower), _MONO_KEYWORDS)
end
