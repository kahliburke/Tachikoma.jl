...
import Tachikoma: enable_raw_capture!, disable_raw_capture!, last_event_raw,
                  reset_key_state!, KITTY_KEYBOARD_ON, KITTY_KEYBOARD_OFF,
                  key_press, key_repeat
...
const SECTIONS = [
    "Control & navigation keys" => [
        ("Enter", "", false, false), ("Tab", "", false, false),
        ("Backspace", "", false, false), ("Escape", "", false, false),
        ("Spacebar", "", true, false),
        ("Delete (forward)", "Fn+Delete on Mac — skip if you have none", false, false),
        ("Up arrow", "", false, false), ("Down arrow", "", false, false),
        ("Left arrow", "", false, false), ("Right arrow", "", false, false),
        ("Home", "", false, false), ("End", "", false, false),
        ("Page Up", "", false, false), ("Page Down", "", false, false),
    ],
    "Produce each LETTER (type it however your layout does)" => [
        ("a", "", false, false), ("z", "", false, false),
        ("q", "", false, false), ("w", "", false, false),
        ("m", "", false, false), ("A   (capital — with Shift)", "", false, false),
    ],
    "Produce each DIGIT" => [
    ...
    "Produce each ACCENTED letter (skip any you can't make)" => [
        ("é", "", false, false), ("è", "", false, false),
        ("à", "", false, false), ("ç", "", false, false), ("ù", "", false, false),
    ],
    "Produce each CODING symbol (essential for programming)" => [
        ("@", "", false, false), ("#", "", false, false),
        ("[", "", false, false), ("]", "", false, false),
        ("{", "", false, false), ("}", "", false, false),
        ("|   (pipe)", "", false, false), ("\\   (backslash)", "", false, false),
        ("~   (tilde)", "", false, false), ("`   (backtick)", "", false, false),
    ...
    "Ctrl combinations  (hold Ctrl, NO Shift; Ctrl+C quits, Ctrl+Z undoes)" => [
        ...
        ("Ctrl + Right arrow", "often an OS shortcut — SKIP if it doesn't reach here", false, false),
    ],
    ...
    "Function keys" => [
        ...
        ("F5", "", false, false), ("F12", "", false, false),
    ],
]

const PROMPTS = Prompt[
    Prompt(section, label, hint, sp, cc)
    for (section, ps) in SECTIONS for (label, hint, sp, cc) in ps
]
...
function utf8_view(bytes)
    s = String(copy(bytes))
    isvalid(s) ? (true, s) : (false, "")
end

_show(c::Char) = isvalid(c) ? "'$(c)'" :
    "U+" * uppercase(string(UInt32(c), base=16, pad=4))

function describe(e::KeyEvent)
    a = e.action == key_press ? "" : e.action == key_repeat ? " [repeat]" : " [release]"
    if e.key == :char
        ":char $(_show(e.char))$a"
    elseif e.char == '\0'
        ":$(e.key)$a"
...
function make_record(p::Prompt, raw, evt, mode)
    ...
    Record(p.section, p.label, mode, copy(raw), valid, utf8, dec, ok, false)
end

function skipped_record(p::Prompt, raw, evt, mode)
    valid, utf8 = utf8_view(raw)
    Record(p.section, p.label, mode, copy(raw), valid, utf8,
           "skipped via $(describe(evt))", nothing, true)
end

free_record(i, raw, evt, mode) = begin
    valid, utf8 = utf8_view(raw)
    ok = (evt isa KeyEvent && evt.key == :char && valid && length(utf8) == 1) ?
         (evt.char == utf8[1]) : nothing
    Record("Free-form", "(free #$(i))", mode, copy(raw), valid, utf8, describe(evt), ok, false)
end
...
function env_report()
    keys = ["TERM","TERM_PROGRAM","TERM_PROGRAM_VERSION","COLORTERM",
            "LANG","LC_ALL","LC_CTYPE","LANGUAGE","KITTY_WINDOW_ID",
            "WEZTERM_EXECUTABLE","ALACRITTY_WINDOW_ID","ITERM_SESSION_ID",
            "SSH_CONNECTION","TMUX","STY"]
...
function finish!(m::KeyDiagModel)
    ...
    (isempty(m.logpath) && !isempty(m.results)) || return
    io = IOBuffer()
    ...
    for r in m.results
        ...
        if r.skipped
            ...
            flag = r.ok === nothing ? "" : r.ok ? "  OK" : "  *** MISMATCH ***"
...
const MODIFIER_KEYS = Set{Symbol}([
    ...
    :left_super, :right_super, :left_hyper, :right_hyper, :left_meta, :right_meta,
...
function render_footer!(m::KeyDiagModel, area, buf)
    ...
    set_string!(buf, area.x + 1, area.y, controls, tstyle(:title, bold=true); max_x=right(area))
end

function render_top!(m::KeyDiagModel, area, buf)
    modestr = m.phase in (:guided,) ? "  ·  mode: $(uppercase(string(curmode(m))))" : ""
    blk = Block(title="Tachikoma keyboard diagnostic$modestr",
                border_style=tstyle(:border), title_style=tstyle(:title, bold=true))
    c = render(blk, area, buf)
    x = c.x + 1; y = c.y

    if m.phase == :intro
        y = putln(buf, x, y, c, "Records the RAW bytes your terminal sends for each key,",
                  tstyle(:text_dim))
        y = putln(buf, x, y, c, "so we can fix keyboard handling for international layouts.",
                  tstyle(:text_dim))
        y += 1
        ks = m.kitty_supported ? ("Kitty keyboard protocol: SUPPORTED", tstyle(:success)) :
    ...
    elseif m.phase == :mode
        ...
        y = putln(buf, x, y, c, "  [1] Kitty mode   (what Kaimon actually uses — recommended)",
                  tstyle(:text))
    ...
    elseif m.phase == :guided
        ...
        set_string!(buf, right(c) - 10, y,
                    lpad("[$(m.idx)/$(length(PROMPTS))]", 10), tstyle(:text_dim))
        y += 2
        ...
        isempty(p.hint) || set_string!(buf, hx + 2, y, "($(p.hint))",
                                       tstyle(:text_dim, dim=true); max_x=right(c))
        putln(buf, x, y + 2, c,
    ...
    elseif m.phase == :freeform
        ...
        y = putln(buf, x, y, c,
                  ...
                  tstyle(:text))
        putln(buf, x, y, c, "Not sure? You're done — just press Ctrl+C.",
              tstyle(:text_dim, dim=true))
    end
end

function render_log!(m::KeyDiagModel, area, buf)
    blk = Block(title="Captured  ($(length(m.results)))",
...
function render_record_line!(buf, x, y, c, r::Record)
    ...
    if r.skipped
        set_string!(buf, x, y, "· $(r.label)  →  $(r.decoded)",
                    tstyle(:text_dim, dim=true); max_x=right(c))
        return
    end
    ...
    if r.ok !== nothing
        mark = r.ok ? " ✓" : " ⚠"
        set_string!(buf, xx, y, mark, r.ok ? tstyle(:success) : tstyle(:error);
                    max_x=right(c))
    end
end
...
function _force_restore_terminal!()
    try Tachikoma.set_raw_mode!(false) catch end          # cooked mode (echo/line editing)
    try
        print(stdout,
