...
rgb_gen = @composed function _rgb(
    r=Data.Integers{UInt8}(), g=Data.Integers{UInt8}(), b=Data.Integers{UInt8}()
)
    T.ColorRGB(r, g, b)
end
...
@testset "PBT: constraint serialization roundtrip" begin
    Supposition.@check function constraint_rt(c=constraint_gen)
        ...
        if c isa T.Fixed
            return c2.size == c.size
        end
        if c isa T.Fill
            return c2.weight == c.weight
        end
        if c isa T.Percent
            return c2.pct == c.pct
        end
        ...
        if c isa T.Ratio
            return c2.num == c.num && c2.den == c.den
        end
        false
    end
end
...
@testset "PBT: Rect contains corners" begin
    ...
    Supposition.@check function inner_fits(r=rect_gen)
        i = T.inner(r)
        i.x >= r.x && i.y >= r.y && T.right(i) <= T.right(r) && T.bottom(i) <= T.bottom(r)
    end
end
