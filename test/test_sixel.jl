...
@testset "Background renders to buffer" begin
    ...
    T.render_background!(bg, buf, area, 10; brightness=0.5, saturation=0.5, speed=1.0)
...
@testset "fb_flush sixel rects avoid text cells" begin
    ...
    fb = T.PixelFramebuffer(
        Vector{UInt8}(undef, pw * ph * 4), pw, ph, true, true, 1, 1, pw, ph, [screen_area]
    )
    for i in 0:(pw * ph - 1)
    ...
    for _ in 1:30
        ...
        T.set_string!(buf, col, row, string(ch), T.Style(fg=T.Color256(196)))
        push!(text_cells, (row, col))
    end

    # Also place a longer string to create a horizontal run of text
    label = "HELLO.TEXT!"
    T.set_string!(buf, 5, 10, label, T.Style(fg=T.Color256(46)))
