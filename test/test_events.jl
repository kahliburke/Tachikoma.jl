...
@testset "Legacy UTF-8 multibyte decoding" begin
    function feed(bytes)
        ...
        T.INPUT_IO[] = IOBuffer(bytes)
        T.INPUT_ACTIVE[] = true
        try
            T.read_event()
        finally
            T.INPUT_IO[] = old_io
            T.INPUT_ACTIVE[] = old_active
        end
    end
    e = feed(UInt8[0x61])
    @test e.key == :char && e.char == 'a'    # ASCII
    e = feed(UInt8[0xc3, 0xa9])
    @test e.key == :char && e.char == 'é'    # 2-byte
    e = feed(UInt8[0xe2, 0x82, 0xac])
    @test e.key == :char && e.char == '€'    # 3-byte
    e = feed(UInt8[0xf0, 0x9f, 0x98, 0x80])
    @test e.key == :char && e.char == '😀'   # 4-byte
