...
function make_test_provider(n::Int=100)
    ...
    return T.InMemoryPagedProvider(cols, data)
end
...
@testset "PagedDataTable: render with block" begin
    pdt = T.PagedDataTable(make_test_provider(50); page_size=10, block=T.Block(title="Test"))
    buf = T.Buffer(T.Rect(1, 1, 60, 20))
    T.render(pdt, T.Rect(1, 1, 60, 20), buf)
    @test pdt.last_content_area.width > 0
end
...
@testset "PagedDataTable: page size change" begin
    pdt = T.PagedDataTable(make_test_provider(100); page_size=50, page_sizes=[25, 50, 100])
...
@testset "PagedDataTable: detail view" begin
    ...
    pdt = T.PagedDataTable(make_test_provider(20); page_size=10, detail_fn=test_detail)
