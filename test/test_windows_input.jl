...
function _mkrec(etype; X=0, Y=0, btn=0, cks=0, flags=0, down=0, vk=0, ch=0)
    ...
    if etype == 2   # MOUSE_EVENT
        put16(5, reinterpret(UInt16, Int16(X)))
        put16(7, reinterpret(UInt16, Int16(Y)))
        put32(9, btn)
        put32(13, cks)
        put32(17, flags)
    else            # KEY_EVENT
...
@testset "windows console input" begin
    ...
    @testset "mouse: wheel" begin
        ...
        down = T._win_translate_mouse(
            _mkrec(2; btn=reinterpret(UInt32, Int32(-120 << 16)), flags=0x4)
        )
        ...
        left = T._win_translate_mouse(
            _mkrec(2; btn=reinterpret(UInt32, Int32(-120 << 16)), flags=0x8)
        )
