...
@testset "Layout" begin
    ...
    # Even split
    rects = T.split_layout(T.Layout(T.Horizontal, [T.Percent(50), T.Percent(50)]), r)
    @test length(rects) == 2
    ...
    # Fixed + Fill
    rects2 = T.split_layout(T.Layout(T.Vertical, [T.Fixed(3), T.Fill()]), r)
    @test rects2[1].height == 3
    @test rects2[2].height == 21
    @test rects2[2].y == 4

    # Three-way with mixed constraints
    rects3 = T.split_layout(T.Layout(T.Horizontal, [T.Fixed(10), T.Fill(), T.Fixed(10)]), r)
...
@testset "Block" begin
    buf = T.Buffer(T.Rect(1, 1, 20, 5))
    block = T.Block(title="test", border_style=T.RESET, title_style=T.RESET)
...
@testset "SelectableList" begin
    ...
    lst = T.SelectableList(["alpha", "beta", "gamma", "delta"]; selected=2)
...
@testset "BarChart" begin
    ...
    bars = [T.BarEntry("cpu", 75.0), T.BarEntry("mem", 45.0), T.BarEntry("disk", 90.0)]
...
@testset "TreeView" begin
    root = T.TreeNode(
        "root",
        [T.TreeNode("child1", [T.TreeNode("leaf1"), T.TreeNode("leaf2")]), T.TreeNode("child2")],
    )
...
@testset "TreeView visible count" begin
    root = T.TreeNode("r", [T.TreeNode("a", [T.TreeNode("x")]), T.TreeNode("b")])
    tv = T.TreeView(root)
