...
@testset "ScrollPane: basic string rendering" begin
    sp = T.ScrollPane(["line1", "line2", "line3", "line4", "line5"]; following=false)
...
@testset "ScrollPane: scrolled rendering" begin
    sp = T.ScrollPane(["line1", "line2", "line3", "line4", "line5"]; offset=2, following=false)
    buf = T.Buffer(T.Rect(1, 1, 20, 3))
    T.render(sp, T.Rect(1, 1, 20, 3), buf)
    # Lines 3-5 visible
...
@testset "ScrollPane: styled Span rendering" begin
    ...
    lines = [[T.Span("hello", red_style)], [T.Span("world", red_style)]]
...
@testset "ScrollPane: mouse scroll changes offset" begin
    ...
    evt_down = T.MouseEvent(10, 3, T.mouse_scroll_down, T.mouse_press, false, false, false)
    @test T.handle_mouse!(sp, evt_down)
    @test sp.offset == 6

    evt_up = T.MouseEvent(10, 3, T.mouse_scroll_up, T.mouse_press, false, false, false)
...
@testset "Layout space_between" begin
    ...
    rects = T.split_layout(
        T.Layout(
            T.Horizontal, [T.Fixed(10), T.Fixed(10), T.Fixed(10)]; align=T.layout_space_between
        ),
        r,
    )
    @test rects[1].x == 1
...
@testset "Layout default is layout_start" begin
    r = T.Rect(1, 1, 100, 24)
    rects = T.split_layout(T.Layout(T.Horizontal, [T.Fixed(20)]), r)
    @test rects[1].x == 1
end
...
@testset "Layout space_around" begin
    ...
    rects = T.split_layout(
        T.Layout(
            T.Horizontal, [T.Fixed(10), T.Fixed(10), T.Fixed(10)]; align=T.layout_space_around
        ),
        r,
    )
    @test length(rects) == 3
...
@testset "Layout space_evenly" begin
    ...
    rects = T.split_layout(
        T.Layout(
            T.Horizontal, [T.Fixed(10), T.Fixed(10), T.Fixed(10)]; align=T.layout_space_evenly
        ),
        r,
    )
    @test length(rects) == 3
...
@testset "Layout positive spacing" begin
    r = T.Rect(1, 1, 100, 24)
    rects = T.split_layout(T.Layout(T.Horizontal, [T.Fill(), T.Fill()]; spacing=10), r)
    @test length(rects) == 2
...
@testset "Layout negative spacing (overlap)" begin
    r = T.Rect(1, 1, 100, 24)
    rects = T.split_layout(T.Layout(T.Horizontal, [T.Fixed(30), T.Fixed(30)]; spacing=-5), r)
    @test length(rects) == 2
...
@testset "Layout Ratio constraint" begin
    r = T.Rect(1, 1, 120, 24)
    rects = T.split_layout(T.Layout(T.Horizontal, [T.Ratio(1, 3), T.Ratio(2, 3)]), r)
    @test length(rects) == 2
...
@testset "Layout single item edge cases" begin
    ...
    rects = T.split_layout(T.Layout(T.Horizontal, [T.Fixed(20)]; spacing=10), r)
    @test rects[1].x == 1
    @test rects[1].width == 20

    # space_around centers single item
    rects2 = T.split_layout(T.Layout(T.Horizontal, [T.Fixed(20)]; align=T.layout_space_around), r)
    @test rects2[1].x == 1 + round(Int, 80 / 2)  # leftover/2n where n=1

    # space_evenly centers single item
    rects3 = T.split_layout(T.Layout(T.Horizontal, [T.Fixed(20)]; align=T.layout_space_evenly), r)
...
@testset "Chart render line" begin
    data = T.DataSeries([(1.0, 1.0), (2.0, 4.0), (3.0, 2.0)]; label="test", chart_type=T.chart_line)
    chart = T.Chart([data])
...
@testset "Chart multi series" begin
    s1 = T.DataSeries([1.0, 2.0, 3.0]; label="s1", style=T.tstyle(:primary))
    s2 = T.DataSeries([3.0, 1.0, 2.0]; label="s2", style=T.tstyle(:accent))
...
@testset "DataTable sorting" begin
    cols = [T.DataColumn("Name", ["bob", "alice", "carol"]), T.DataColumn("Val", [2, 1, 3])]
    dt = T.DataTable(cols)
    T.sort_by!(dt, 2)
...
@testset "DataTable col_widths override" begin
    cols = [
        T.DataColumn("Name", ["alice", "bob"]), T.DataColumn("Score", [95, 87]; align=T.col_right)
    ]
    dt = T.DataTable(cols; selected=1)
...
@testset "DataTable col_widths initialized on first render" begin
    cols = [T.DataColumn("Name", ["alice", "bob"]), T.DataColumn("Score", [95, 87])]
    dt = T.DataTable(cols; selected=1)
...
@testset "DataTable border positions cached" begin
    cols = [
        T.DataColumn("A", ["x", "y"]), T.DataColumn("B", ["1", "2"]), T.DataColumn("C", ["a", "b"])
    ]
    dt = T.DataTable(cols; selected=1)
    buf = T.Buffer(T.Rect(1, 1, 40, 8))
    T.render(dt, T.Rect(1, 1, 40, 8), buf)
...
@testset "DataTable mouse drag resize" begin
    ...
    press_evt = T.MouseEvent(border_x, header_y, T.mouse_left, T.mouse_press, false, false, false)
    @test T.handle_mouse!(dt, press_evt, area)
    @test dt.col_drag == col_idx

    # Drag to the right
    start_w = dt.col_drag_start_w
    drag_evt = T.MouseEvent(border_x + 5, header_y, T.mouse_left, T.mouse_drag, false, false, false)
    @test T.handle_mouse!(dt, drag_evt, area)
    @test dt.col_widths[col_idx] == start_w + 5

    # Release
    release_evt = T.MouseEvent(
        border_x + 5, header_y, T.mouse_left, T.mouse_release, false, false, false
    )
...
@testset "DataTable mouse drag via convenience overload" begin
    ...
    press = T.MouseEvent(border_x, header_y, T.mouse_left, T.mouse_press, false, false, false)
    @test T.handle_mouse!(dt, press)
    @test dt.col_drag == col_idx

    drag = T.MouseEvent(border_x + 3, header_y, T.mouse_left, T.mouse_drag, false, false, false)
    @test T.handle_mouse!(dt, drag)

    release = T.MouseEvent(
        border_x + 3, header_y, T.mouse_left, T.mouse_release, false, false, false
    )
...
@testset "DataTable drag trailing border resizes last column" begin
    ...
    drag = T.MouseEvent(border_x + 4, header_y, T.mouse_left, T.mouse_drag, false, false, false)
    T.handle_mouse!(dt, drag, area)
    @test dt.col_widths[2] == rendered_w + 4

    release = T.MouseEvent(
        border_x + 4, header_y, T.mouse_left, T.mouse_release, false, false, false
    )
...
@testset "Form tab navigation" begin
    ...
    fields = [T.FormField("F1", ti), T.FormField("F2", cb)]
    form = T.Form(fields)
    # Tab moves focus
...
@testset "form value()" begin
    ...
    cb = T.Checkbox("x"; checked=true)
    fields = [T.FormField("Name", ti), T.FormField("Agree", cb)]
    form = T.Form(fields)
    vals = T.value(form)
