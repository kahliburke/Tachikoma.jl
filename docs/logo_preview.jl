...
function _render_logo_v3!(
    buf::Buffer, tx::Int, logo_y::Int, tick::Int, rect_bottom::Int, rect_right::Int
)
    th = theme()
    ...
    for (row_i, line) in enumerate(_LP_LOGO_DATA)
        for col_i in 1:length(line)
            ...
            n = 0.5 + 0.3 * sin(τ * t * 0.5 + col_i * 0.07) + 0.2 * cos(τ * t + row_i * 0.5)
...
function view(m::LogoPreview3, f::Frame)
    ...
    return render(
        ...
        Rect(area.x, bottom(area), area.width, 1),
        buf,
    )
end
